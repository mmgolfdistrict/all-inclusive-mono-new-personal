...
export const ForecastModal = ({ closeForecastModal, startDate: propStartDate, endDate: propEndDate }: Props) => {
    ...
    const getNext7Days = (currentStartDate: Date | string, propEndDate: string): string => {
    ...
    return (
        <div className="fixed inset-0 z-20 flex items-center justify-center bg-black w-screen backdrop-blur bg-opacity-50 overflow-hidden">
            <div className={`bg-white p-6 rounded-lg  sm:max-w-4xl md:max-w-3xl lg:max-w-2xl xl:max-w-5xl ${isMobile && "w-full"}`}>
                <div className="flex justify-between items-center">
                    <div className="flex justify-between items-center mb-2 w-[170px]">
                        ...
                        <button
                            className="text-sm font-medium hover:underline"
                            onClick={handleNext}
                            disabled={new Date(endDate) >= new Date(propEndDate)}
                        >
                            Next &gt;
                        </button>
                    </div>
                    ...
                    <table className=" w-full border-collapse text-sm">
                        <thead className="top-0 table-header-group">
                            <tr className="text-left">
                                {!isLoading && <TableHeader text="Dates / Times" />}
                                {data?.map((item) => (
                                    <TableHeader
                                ...
                                : <>
                                    <tr className="border-b border-stroke text-primary-gray">
                                        <td className="px-4 py-2 font-semibold">
                                            <p className="text-secondary-black">Early Morning</p> 06:00 AM – 07:30 AM
                                        </td>
                                        {data?.map((item) => (
                                            <td key={item.providerDate} className="px-4 py-2 text-center">
                                    ...
                                    <tr className="border-b border-stroke text-primary-gray">
                                        <td className="px-4 py-2 font-semibold">
                                            <p className="text-secondary-black">Mid-Morning</p> 08:00 AM – 10:30 AM
                                        </td>
                                        {data?.map((item) => (
                                            <td key={item.providerDate} className="px-4 py-2 text-center">
                                                {item.MidMorning != null && item.MidMorning >= 0
                                    ...
                                    <tr className="border-b border-stroke text-primary-gray">
                                        <td className="px-4 py-2 font-semibold">
                                            <p className="text-secondary-black">Early Afternoon</p> 10:30 AM – 1:30 PM
                                        </td>
                                        {data?.map((item) => (
                                            <td key={item.providerDate} className="px-4 py-2 text-center">
                                    ...
                                    <tr className="border-b border-stroke text-primary-gray">
                                        <td className="px-4 py-2 font-semibold">
                                            <p className="text-secondary-black">Afternoon</p> 02:00 PM – 04:00 PM
                                        </td>
                                        {data?.map((item) => (
                                            <td key={item.providerDate} className="px-4 py-2 text-center">
                                    ...
                                    <tr className="border-b border-stroke text-primary-gray">
                                        <td className="px-4 py-2 font-semibold">
                                            <p className="text-secondary-black">Twilight</p> 04:00 PM – 06:00 PM
                                        </td>
                                        {data?.map((item) => (
                                            <td key={item.providerDate} className="px-4 py-2 text-center">
...
const TableHeader = ({ text, className }: { text: string; className?: string }) => {
    return (
