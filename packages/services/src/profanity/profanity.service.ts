...
export class ProfanityService {
    ...
    isProfane = async (text: string) => {
        try {
            ...
            const matchingWords = await this.db.select().from(profanities).where(or(sql`${text} like CONCAT('%', profanityText, '%')`, like(profanities.profanityText, `%${text}%`))).catch((err) => {
                this.logger.error(err);
                throw new Error(err);
            });
            const matchingPhonetically = await this.db.select().from(profanities).where(sql`SOUNDEX(profanityText)=SOUNDEX(${text})`).catch((err) => {
                this.logger.error(err);
                throw new Error(err);
            })
