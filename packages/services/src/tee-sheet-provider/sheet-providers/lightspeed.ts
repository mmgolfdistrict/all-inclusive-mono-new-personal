...
export class Lightspeed extends BaseProvider {
    ...
    fetchTeeTimes = async (courseId: string, date: string, providerConfiguration: any, page: number) => {
        try {
            const { BASE_ENDPOINT, CONTENT_TYPE, ORGANIZATION_ID, ACCEPT, DEFAULT_PLAYER_TYPE } = JSON.parse(providerConfiguration ?? "{}");
            ...
            const url = `${BASE_ENDPOINT}/partner_api/v2/organizations/${ORGANIZATION_ID}/teetimes?custom_params[player_count]=4&custom_params[holes]=18&custom_params[with_pricing]=true&custom_params[player_types]=${DEFAULT_PLAYER_TYPE}&filter[course]=${courseId}&page[size]=100&page[number]=${page}&filter[date]=${date}`;
            const headers = {
    ...
    async createBooking(
        ...
        const { BASE_ENDPOINT, CONTENT_TYPE, ORGANIZATION_ID, ACCEPT, DEFAULT_PLAYER_TYPE } = JSON.parse(this.providerConfiguration ?? "{}");
        if (!token) {
