...
export const getCourseById = async (courseId: string) => {
  ...
  try {
    ...
    console.log("Course Fetch Error: ", JSON.stringify(error));
  }
};
...
export const processFinixWebhook = async (webhookData: any) => {

  /**================= */
  // Inilize hyperswitch service 
  const appSettingService = new AppSettingsService(db, process.env.REDIS_URL!, process.env.REDIS_TOKEN!);
  ...
  const entityType = webhookData?.type;
  const entity = webhookData?.entity;
  // const paymentId = webhookData?._embedded?.payment_links[0]?.id || "";
  const paymentId = webhookData?._embedded?.payment_links?.[0]?.id ?? null;
  const paymentState = webhookData?._embedded?.payment_links?.[0]?.state ?? null;
  const paymentCompleteData = JSON.stringify(webhookData);
  if (paymentId) {
    await hyperSwitchWebhookService.finixWebhookService(entityType, entity, paymentId, paymentState)
...
export const trackEmailOpen = async (id: string) => {
  //======================
  const appSettingService = new AppSettingsService(db, process.env.REDIS_URL!, process.env.REDIS_TOKEN!);
  ...
  //=====================
  if (id) {
    await hyperSwitchWebhookService.updateTrackStatusOfOpenedEmail(id)
      .then((data) => console.log("updated successfully", data))
      .catch((error) => console.log("error", error))
  } else {
