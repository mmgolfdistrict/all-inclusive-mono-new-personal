...
export const formatTimeWithoutDate = (timestamp: string, showFullDayOfTheWeek?: boolean, utcOffset = 0): string => {
  const cleanTimeString = !timestamp.includes("T") ? timestamp.replace(" ", "T") + "Z" : timestamp;
  const timezone = cleanTimeString.slice(-6) ?? utcOffset;
  if (showFullDayOfTheWeek) {
    return dayjs.utc(cleanTimeString).utcOffset(timezone).format("hh:mm A");
  }
  return dayjs.utc(cleanTimeString).utcOffset(timezone).format("hh:mm A");
};
